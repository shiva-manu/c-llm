 ...
 * openai_compat.c
 *
 * Shared implementation for providers that use the OpenAI Chat Completions
 * API format. This includes Groq, Together AI, Perplexity, DeepSeek,
 * Fireworks AI, xAI, Cerebras, SambaNova, OpenRouter, and Ollama.
 */
...
LLMResponse openai_compat_generate(const char *url, const char *env_var,
    ...
    if (req->stream && req->stream_cb) {
        ...
        int rc = http_post_stream(url, headers, body, compat_stream_chunk, &ctx);
