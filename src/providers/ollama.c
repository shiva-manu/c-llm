 ...
 *
 * Env: OLLAMA_API_KEY (optional, defaults to "ollama" if unset)
 * URL defaults to localhost:11434
 */
...
static int ollama_stream_chunk(const char *data, size_t size, void *user_data) {
    struct ollama_stream_ctx *ctx = (struct ollama_stream_ctx *)user_data;
...
LLMResponse ollama_generate(const LLMRequest *req) {
    ...
    if (req->stream && req->stream_cb) {
        ...
        int rc = http_post_stream("http://localhost:11434/v1/chat/completions",
                                  headers, body, ollama_stream_chunk, &ctx);
    ...
    char *response = http_post("http://localhost:11434/v1/chat/completions", headers, body);
    free(body);
    curl_slist_free_all(headers);

    if (!response) { res.error = "HTTP request failed (is Ollama running?)"; return res; }
